...
public class String_1 {
    ...
    "X", and the empty string "" yields the empty string "". Note that str.length() returns the length of a string.
    firstTwo("Hello") → "He"
    firstTwo("abcdefg") → "ab"
    firstTwo("ab") → "ab"  */
    ...
    public boolean hasBad(String str) {
        if (str.length() >= 3) {
            ...
            if (str.length() >= 4 && str.startsWith("bad", 1))
                return true;
        }
        return false;
    }
    ...
    char of b, so "yo" and "java" yields "ya". If either string is length 0, use '@' for its missing char.
    ...
    /* Given two strings, append them together (known as "concatenation") and return the result. However,
    if the strings are different lengths, omit chars from the longer string so it is the same
